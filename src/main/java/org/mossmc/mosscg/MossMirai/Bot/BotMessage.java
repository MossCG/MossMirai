...
import org.mossmc.mosscg.MossMirai.Sender;
...
public class BotMessage {
    ...
    public static void readMessage(Group group, User user, MessageChain chain, messageType type) {
        ...
        if (group != null) {
            if (!BotManager.groupList.contains(group.getId())) {
                return;
            }
        } else {
            if (!BotManager.adminList.contains(user.getId())) {
                return;
            }
        }
        MessageChainBuilder reply;
        reply = BotCommand.readCommand(chain,user.getId());
        if (reply != null) {
